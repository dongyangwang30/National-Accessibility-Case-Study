...
# The goal of this section is to understand the structure of the variables and find 
# potential correlations for model fitting. This section is especially interested in 
# finding variables that might correlate with the existence of ramps in the events.

### 1.1 EDA
setwd("/Users/dongyangwang/Desktop/Skill/Data Camp/Certificate")

# Clean the environment
#rm(list=ls())

# Load the data
df <- read.csv("event_venues.csv")
...
# In the EDA, we found that there are 3910 observations and 10 variables in the dataset.
# Most of the variables are converted to logical variables to facilitate the understanding
# of their structures, including the presence of loud music, alcohol, WIFI, super venue, 
# promotion, and wheelchair accessibility. The capacity variables are kept numeric. 
# One advantage of this dataset is that exactly half of the data are wheelchair accessible, 
# so analysis can be easily performed. Other variables are not so well distributed, 
# such as only 7% of the events do not provide WIFI.
...
# Given the visualizations, only total standing capacity seems to make difference. 
# U-shaped and theater capacities do not exhibit a clear pattern for wheelchair accessibility, 
# due to large amount of missing actual values. If we only consider true values, 
# all three variable show that larger capacity in any regard is associated with wheelchairs accessibility.
...
# Based on visualizations, the presence of WIFI, loud music, alcohol, and promotional tickets 
# is associated with better wheelchair accessibility. If the venue is a supervenue, 
# it is less likely to have wheelchair accessibility.

## 2. Model

# The goal of this section is to fit a logistic model on the data to be able to 
# make predictions for ramp availability (wheelchair accessibility) based on other variables. 
# It is particularly concerned that at least two-thirds of venues predicted to be without 
# a ramp should not have a ramp. The model is evaluated accordingly: the focus is 
# on the negative predictive value (npv), while not sacrificing too much accuracy.

### 2.1 Model Fitting

# I fit two models because of the incomplete information in two variables, U.Shaped_max 
# and Theatre_max. The purpose is to be able to make predictions in a more precise manner. 
# If we do have accurate information for those two variables, it is desirable that 
# we use the second model. Otherwise, we can input the mean values for those two variables 
# and make a prediction using the first model.

#### Model with all values

# First I fit a model with all values in the dataset. Step-wise regression is used 
# to find the optimal regression model.
...
# Then I fit a model with only true values (in terms of two variables, namely U.Shaped_max and 
# Theatre_max) in the dataset. This model contains roughly 1/4 of the observations in the original dataset. Step-wise regression is used to find the optimal regression model.
...
# Comparing the two models, we can find that a few variables are reliable in predicting 
# wheelchair accessibility. The presence of alcohol, the event being promoted or ticketed 
# are the presence of WIFI positively correlated with wheelchair accessibility; the venue 
# as supervenue and the presence of loud music are negatively correlated with wheelchair accessibility.
# The capacity variables have mixing results and depend on the specific model. 
# Therefore, in general, we would look for events where no alcohol, WIFI, or ticketing is present, 
# as well as events where the venue is supervenue and where loud music is played.


### 2.2 Model Evaluation

# The key to evaluate the two models is how good their predictions are when venues 
# are not predicted to be with a ramp. ROC curves are also used to assist model evaluation.

#### Model with all values

# Threshold is set to 0.4 to control the False Negative rates, such that the result 
# for negative predictive value will be higher, without sacrificing too much accuracy.
...
# Therefore, the model with the actual values is better at predicting wheelchair
# accessibility when the predicted value is 0. But we still need the first model 
# to be able to account for the situation where the capacity information is missing.


## 3. Summary

# The goal of this section is to summarize the analysis and discuss its impacts. 
# It's also concerned to provide insights on how future steps might improve the 
# results and how to make best use of the results.

### 3.1 Main Findings
# Two models can be applied depending on whether we have complete information on 
# the capacity information, and either model will be able to provide the right 
# suggestion 2/3 of the time. Also, we should take time prioritizing events without 
# alcohol, WIFI, ticketing or promotion. At the same time, we want to search for 
# super venue events and events with loud music. 

### 3.2 Further Steps
# First, we can keep collecting data, preferably data with capacity information 
# because the second model is better. We may also seek to gather new information 
# on the type of host, e.g., NGO or corporate, and the type of the event, e.g., 
# a show or concert. We can improve our models in this manner.

# Second, we should prioritize reaching to events with the characteristics we just 
# found. Then we are likely to give help to people who really need the ramps and 
# not to make our company look bad.
